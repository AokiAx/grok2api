...
<!-- AUTO-GENERATED: source=backend/cmd/grok2api/main.go -->
| 命令 | 说明 |
|------|------|
...
| `bootstrap-admin` | 从 stdin 一次性创建本地管理员；必须使用 `--password-stdin` |
<!-- /AUTO-GENERATED -->
...
```powershell
# 导出全部账号
go -C backend run ./cmd/grok2api export --config ../config.json

# 只导出 ready 池并指定文件
go -C backend run ./cmd/grok2api export --config ../config.json --pool ready --out ../data/ready_accounts.json
```

`--pool` 可取 `ready` / `unavailable`，留空表示全部。旧的 `register` / `mint` 命令已迁移到外部 `grok-register`，在本程序中调用会直接返回迁移提示。

首次初始化管理员推荐使用 stdin，避免密码进入 shell history 或进程参数：

```powershell
$env:GROK2API_ADMIN_PASSWORD = "请替换为至少 12 个字符的密码"
$env:GROK2API_ADMIN_PASSWORD | go -C backend run ./cmd/grok2api bootstrap-admin --config ../config.json --password-stdin
Remove-Item Env:GROK2API_ADMIN_PASSWORD
```
...
- 使用管理员用户名和密码登录；默认用户名为 `admin`
- access token 仅保存在前端内存中，不写入 `localStorage` / `sessionStorage`
- refresh token 仅存于 `HttpOnly`、`SameSite=Strict` cookie，并在刷新时轮换
- 勾选 **记住登录** 只决定 refresh cookie 是否跨浏览器会话持久化；服务端会话最长 30 天
- access token 有效期 5 分钟；401 时前端最多 single-flight 自动刷新一次
- 点「退出」会撤销服务端会话并删除 refresh cookie
...
<!-- AUTO-GENERATED: source=backend/internal/api/server.go -->
| 方法 | 路径 | 说明 |
|---|---|---|
...
`api_key` 不再具备管理员权限。首次 `serve` 会将它单向迁移成数据库中的 `legacy` 客户端密钥；撤销后重启不会复活。`panel_password` 优先、`app_key` 兜底，仅用于兼容旧配置的一次性管理员迁移，运行时管理 API 只接受管理员 session。

新部署推荐使用 `bootstrap-admin --password-stdin`，并将 `panel_password` / `app_key` 留空。`admin_secure_cookies=true` 仅用于 HTTPS 或 TLS 反向代理部署；纯 HTTP 下设置为 true 会导致浏览器不发送 refresh cookie。
...
| `admin_secure_cookies` | refresh cookie 增加 `Secure`；仅 HTTPS/TLS 反代启用 | `false` |
...
<!-- AUTO-GENERATED: source=backend/internal/config/config.go -->
| 类型 | 环境变量 |
|------|----------|
| 字符串 | `GROK2API_HOST`、`GROK2API_API_KEY`、`GROK2API_APP_KEY`、`GROK2API_PANEL_PASSWORD`、`GROK2API_PROXY_BASE_URL`、`GROK2API_CLIENT_VERSION`、`GROK2API_CLIENT_IDENTIFIER`、`GROK2API_CLIENT_USER_AGENT`、`GROK2API_TOKEN_AUTH`、`GROK2API_CREDENTIAL_KEY`、`GROK2API_DEFAULT_MODEL`、`GROK2API_DATA_DIR`、`GROK2API_PROXY`、`PROXY_URL`、`GROK2API_DEBUG_TRACE_DIR`、`GROK2API_FRONTEND_STATIC_PATH` |
...
| 布尔 | `GROK2API_ADMIN_SECURE_COOKIES`、`GROK2API_DEBUG_TRACE`、`GROK2API_DEBUG_TRACE_ERRORS_ONLY`、`GROK2API_CLI_POOL_STICKY` |
<!-- /AUTO-GENERATED -->
...
任意命令首次打开数据库时都会幂等执行 schema 迁移；无需先单独执行 `migrate`。除 `bootstrap-admin` 外，现有运行命令还会兼容导入旧账号文件：

- Python SQLite v1 `cli_accounts` → Go `accounts`（当前 schema v5）
...
go -C backend vet ./...
go -C backend build ./cmd/grok2api

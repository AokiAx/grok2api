...
上游是 `cli-chat-proxy.grok.com`（CLI 凭证链路），不是 grok.com 网页 SSO。本服务负责 **导入凭证、号池调度、兼容 API**；批量注册已拆到独立项目 [`grok-register`](../grok-register)。请仅使用你有权操作的账号资源，并遵守上游服务条款。
...
仓库 `docker-compose.yml` 可编排代理侧车，供本服务出口与外部注册机共用：
...
```powershell
Copy-Item config.example.json config.json
# 建议：GROK2API_PROXY=http://privoxy:8118
docker compose up -d
```
...
- 应用只认代理 URL（`proxy` / `GROK2API_PROXY`）
...
| `proxy` | 出口代理 | 空；compose 常用 `http://privoxy:8118` |

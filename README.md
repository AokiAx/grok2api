...
注册机已拆到 **`grok-register`**（与本仓同级：`../grok-register`），本服务只负责 **导入凭证 + 号池调度**。

```powershell
# 独立注册机
cd ../grok-register
go run ./cmd/grok-register register --config config.json --count 10 --workers 2
# 配置 grok2api_base_url + grok2api_api_key 可自动 POST 导入
# 或面板 /panel 粘贴 output/import/*.json
```

详见 `../grok-register/README.md`。
